...
source("/home/fbalao/Datos/ARTICULO/Dacthylorhizapolyploids/RUVseqModel_FB.R")
load("~/Datos/ARTICULO/Dacthylorhizapolyploids/ResultsRUV_bio3.RData") #This load the bioclim variables
colnames(data)[1:4]<-c("tB1830_1", "tB1830_2", "tB1833_1", "tB1833_2")
species = c()
...
bio3<-findEdgeRgenes(model = ~biodata3$bio_5+normSet$W, group = NULL, set = set, pvalue = 0.05)
...
plot(biodata3$bio_5, counts(set)['Cluster-5138.0',] , cex=1, pch=16, ylab="Normalized counts", xlab="bio5", col=c(2,3)[colspecies])
legend(210,189,legend = c("D. majalis", "D. traunsteineri"), col=c(2,3), pch=16, cex=0.7)
